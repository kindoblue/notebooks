...
def equal(u, v):
    ...
    for key in u.D:
        if u.f.get(key, 0) != v.f.get(key, 0):
            return False
...
def add(u, v):
    ...
    # first create a copy of the first vector
    z = u.copy()

    # iterate on the keys of the second vector and sum
...
def dot(u, v):
    ...
    # alias for the most sparse vector
    vv = v

    # check if v is the most sparse. If not,
    # set vv to u
    if len(v.f.keys()) > len(u.f.keys()):
        vv = u

    # sum the list of products
    return sum([u[key]*v[key] for key in vv.f.keys()])
